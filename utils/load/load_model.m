function model = load_model(class_uid, model_uid, varargin)
% Function to load model from dataset
%
%   To load model from ShapeNet with automatically alignment, run
%       model = load_model(class_uid, model_uid, 'AlignedAuto', true);
%
%   To load model from ShapeNet aligned to reference model, run
%       model = load_model(class_uid, model_uid, 'AlignRefModel', modelRef);
%
%   To load one model from PASCAL3D+, run
%       model = load_model(cls, model_idx, 'dataset', 'pascal');
%
%   To load all models from PASCAL3D+, run
%       models = load_model(cls, [], 'dataset', 'pascal');
    ...
    case 'pascal'
        ...
        if isempty(model_uid)
            ...
            for i = 1:numel(mat_content.(class_uid))
                ...
                model{i}.model_uid = i;
                model{i}.vertices = object.vertices;
                model{i}.faces = object.faces;
                ...
                for j = 1:numel(model{i}.anchor_names)
                    if ~isempty(object.(model{i}.anchor_names{j}))
                        dist = pdist2(object.(model{i}.anchor_names{j}), model{i}.vertices);
                        [~, anchor] = min(dist);
                ...
                % Note that this is a temporary solution
                invalid_mesh = [];
                for j = 1:size(model{i}.faces, 1)
                    if rank(model{i}.vertices(model{i}.faces(j, :), :)) ~= 3
                        invalid_mesh = [invalid_mesh; j];
                    end
                end
                model{i}.faces(invalid_mesh, :) = [];
                if option.load_info
            ...
            model.vertices = object.vertices;
            model.faces = object.faces;
            model.dataset = option.dataset;
            ...
            for i = 1:numel(model.anchor_names)
                if isempty(object.(model.anchor_names{i}))
                    ...
                    dist = pdist2(object.(model.anchor_names{i}), model.vertices);
                    [~, anchor] = min(dist);
