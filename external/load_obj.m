function [V,F3,F4] = load_obj(modelname, opts)
% load_obj load a Wavefront/Alias obj style model.
%    It will only consider polygons with 3 or 4 vertices.
%    [v,f3,f4]=load_obj(modelname)
%    s=load_obj(modelname)
%    s=load_obj(modelname,1) % will load vertex normals and textures
...
%
% Examples:
%   load_obj('name.obj') will load and display name.obj
%   S=load_obj('name.obj') will load obj into S
%   patch('Vertices',S.v','Faces',S.f3','FaceColor','g');
%
%
% s=load_obj(modelname) gives a structure
%  version
%  vertices
%  f3 (triangles)
%  f4 (quadrilaterals)
